...
CanDev::CanDev(
        std::string pdcan_name, 
        TunnelType  dev_type  = TunnelType::USBCAN_II_TYPE,       // Assume by default we wat a GC USBCAN-II C
        int         dev_index = 0, 
        int         can_index = 0)
{
    _name = pdcan_name;
...
/// @brief 
/// @param length       Length of data expected to receive, in byte
/// @param wait_time    wait_time param in Receive funciton
/// @return             CAN data in vector<unit8_t>, or empty vector if error
